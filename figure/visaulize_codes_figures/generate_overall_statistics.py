...
def load_code_mapping():
    """加载Code到标准名称的映射关系"""
    mapping_file = Path("code_mapping.json")
    if mapping_file.exists():
        with mapping_file.open("r", encoding="utf-8") as f:
            return json.load(f)
    else:
        print("警告: 未找到 code_mapping.json 文件")
        return {}
...
def load_simplified_code_mapping():
    """加载将各种原始/变体 code 归一到数值分级 code 的映射 (simplified_code.json)"""
    simp_file = Path("simplified_code.json")
    if simp_file.exists():
        with simp_file.open("r", encoding="utf-8") as f:
            return json.load(f)
    else:
...
def load_records(root: Path, code_mapping, simplified_mapping, model_name=""):
    ...
    for fp in iter_annotation_files(root):
        try:
            ...
            simplified_code = simplified_mapping.get(code, code)
...
def compute_overall_code_level_proportions(all_records, simplified_mapping):
    ...
    for r in all_records:
        raw = r.get("code") or ""
        mapped = simplified_mapping.get(raw, raw)
        if not is_numeric_code(mapped):
            continue
...
def create_pie_chart(counter: defaultdict, title: str, out_path: Path, min_percentage=1, fix_overlap=False, startangle=120):
    ...
    for (code, name), cnt in items_by_value:
        ...
        if percentage >= min_percentage and name != "Other":
    ...
    main_items.sort(key=lambda item: natural_sort_key(item[0][0]))
    ...
    # 准备数据
    labels_with_codes = [item[0] for item in main_items]
    sizes = [item[1] for item in main_items]
    total = sum(sizes) # 重新计算总数

    # --- 2. 根据大类生成颜色 (Color logic remains the same) ---
    codes = [lc[0] for lc in labels_with_codes]
    is_success_chart = any(str(c).startswith('S') for c in codes if c and c != "Other")

    major_categories = sorted(list(set([str(c).split('.')[0] for c in codes if c != "Other"])), key=natural_sort_key)
    ...
    major_color_map = {cat: color for cat, color in zip(major_categories, base_palette)}
    ...
    for code, name in labels_with_codes:
        ...
        major_cat = str(code).split('.')[0]
        if major_cat not in sub_category_shades:
            count = sum(1 for c, n in labels_with_codes if c != "Other" and str(c).startswith(major_cat))
            base_color = major_color_map.get(major_cat, "grey") # Fallback
            # 创建阴影
            shades = sns.light_palette(base_color, n_colors=count + 2, reverse=False)[1:]
            sub_category_shades[major_cat] = list(shades)
...
def main():
    ...
    annot_root = project_root / "annotation"
    out_dir = project_root / "taxonomy_statistics"
    ...
    simplified_mapping = load_simplified_code_mapping()
    print(f"加载了 {len(simplified_mapping)} 个简化Code映射关系")

    all_records = []

    deepseek_root = annot_root / "deepseek-v3+gpt-5"
    if deepseek_root.exists():
        ...
        print("警告: 未找到 deepseek-v3+gpt-5 目录")
    ...
    success, failure, total_success, total_failure, model_stats, _ = aggregate_all_records(all_records)
    ...
    overall_level_stats = compute_overall_code_level_proportions(all_records, simplified_mapping)
