...
DEFAULT_MODERATOR_MODEL = "deepseek-v3-official" # 分类/招募用的文本模型
DEFAULT_RECRUITER_MODEL = "deepseek-v3-official" # 招募专家用的文本模型
DEFAULT_AGENT_MODEL = "deepseek-v3-official" # 分析智能体的默认多模态模型
...
class BaseAgent:
    ...
    def call_llm(self,
        ...
        while retries < max_retries:
            try:
                ...
                if messages[-1]['role'] == 'user':# 这意味着我们正在处理一个正常的用户查询
                    self.memory.append(messages[-1]) # 储存用户的问题
                    self.memory.append({"role": "assistant", "content": response}) # 储存我们的回答
                return response
    ...
    def chat(self,
        ...
        messages.append(user_message) # Add the current prompt # 使用过去的记忆
...
class Group:
    ...
    def __init__(self,
        ...
        if not self.lead_agent and members:
            ...
            self.lead_agent = members[0] # 没有领导者就指定第一个成员为领导者
        elif not members:
    ...
    def perform_internal_discussion(self) -> str:
        ...
        # Generate the final group report，由lead agent生成最终报告
...
class MDAgentsFramework:
    ...
    def __init__(self,
        ...
        Args:
            ...
            max_rounds_intermediate: Max discussion rounds for intermediate complexity. 中等复杂度的最大讨论轮数
            max_turns_intermediate: Max turns per round for intermediate complexity. 中等复杂度下的每轮的最大对话次数
            num_experts_intermediate: Number of experts to recruit for intermediate. 中等复杂度下招募的专家数量
            num_teams_advanced: Number of teams to recruit for advanced. 高级复杂度下招募的团队数量
            num_agents_per_team_advanced: Number of agents per team for advanced. 高级复杂度下每个团队的智能体数量
        """
        # 看起来是根据任务复杂度灵活调整智能体数量和讨论轮数
    ...
    def _determine_complexity(self, question: str, options: Optional[Dict] = None, image_path: Optional[str] = None) -> ComplexityLevel:
        ...
        try:
            ...
            if complexity_str == "basic":
                ...
                return ComplexityLevel.INTERMEDIATE # 出问题了就默认为中等复杂度
    ...
    def _recruit_experts(self,
        ...
        # --- ADVANCED RECRUITMENT ---
            ...
            # 复杂任务引入了两阶段的团队协作：一开始的评估队伍以及最后的决策队伍
            recruitment_instruction = (
    ...
    def _process_intermediate_query(self, data_item: Dict, expert_configs: List[Dict]) -> Dict:
        ...
        # 2. Initial Opinions (Round 1)，手动进行多轮对话
        ...
        # Setup interaction log structure,设置json格式的交互日志
    ...
    def _process_advanced_query(self, data_item: Dict, team_configs: List[Dict]) -> Dict:
        ...
        groups = []
        group_dict = {}
        all_agents_in_groups = []  # Keep track of all agents created，这个跟踪没啥意义，很乱
...
def main():
    ...
    parser.add_argument("--dataset", type=str,default='MedQA', help="Specify dataset name (e.g., vqa_rad, pathvqa, medqa)")
