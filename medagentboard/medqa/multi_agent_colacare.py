...
class DoctorAgent(BaseAgent):
    ...
    def analyze_case(self,
        ...
        # Add image if provided，是否解析图片的开关
        if image_path:
            base64_image = encode_image(image_path)
            user_content.append({
                "type": "image_url",
                # data: - Data URL的协议标识符
                # image/jpeg - MIME类型，表示图片格式
                # base64, - 编码方式声明
                # {base64_image} - 实际的Base64编码图片数据
    ...
    def review_synthesis(self,
        ...
        try:
            ...
            if isinstance(result.get("agree"), str):
                result["agree"] = result["agree"].lower() in ["true", "yes"]#统一回答为True/False 
...
class MetaAgent(BaseAgent):
    ...
    def synthesize_opinions(self,
        ...
        # For multiple choice questions, instruct to choose one option，如果是多选题加上这些描述，让他一定给出一个选项。
        if options:
            system_message["content"] += (
        ...
        # Format doctors' opinions as input ，格式化多个医生建议以及专业方向。
        formatted_opinions = []
    ...
    def make_final_decision(self,
        ...
        # Format doctor reviews，一次性收集多个医生智能体的评价。
        formatted_reviews = []
        ...
        decision_type = "final" if all_agree or reached_max_rounds else "current round" # 判断是否是最终决策

        # Review previous rounds' syntheses from memory,如果有多轮的话记录之前的信息。
        previous_syntheses = []
        ...
        # user_message提供具体问题和数据、在一轮的meta决策时机被触发，因此会影响单次回答内容。
        user_message = {
            "role": "user",
            "content": f"Question: {question_with_options}\n\n" # 问题
        ...
        # Call LLM with retry mechanism，调用LLM，metaagent结合到此轮的所有信息做出综合决策。
        response_text = self.call_llm(system_message, user_message)
...
class MDTConsultation:
    ...
    def __init__(self,
        ...
        self.doctor_agents = [] # 创建一个医生智能体列表
        for idx, config in enumerate(self.doctor_configs, 1): # 从1开始编号
    ...
    def run_consultation(self,
        ...
        while current_round < self.max_rounds and not consensus_reached:
            ...
            for i, doctor in enumerate(self.doctor_agents):
                ...
                doctor_opinions.append(opinion) # 医生的意见包括explaination 和answer
            ...
            # Step 3: Doctors review synthesis 每个智能体评审meta智能体的综合意见
            doctor_reviews = []
            all_agree = True
            for i, doctor in enumerate(self.doctor_agents):
                ...
                agrees = review.get('agree', False) # 每个doctor具有一票否决权
        ...
        # 安全防护措施，如果确实没有 final_decision，至少返回最后一轮的 decision
...
def parse_structured_output(response_text: str) -> Dict[str, str]:
    ...
    try:
        ...
        for line in lines:
            if ":" in line:
                key, value = line.split(":", 1) # 只取第一个冒号前后的内容
                key = key.strip().lower()
                value = value.strip()
                result[key] = value
...
def process_input(item, doctor_configs=None, meta_model_key="qwen-max-latest"):
    ...
    # Run consultation
    # 这个result包括最终的决策、是否达成共识、第几轮、处理的总时间等信息。
...
def main():
    ...
    parser.add_argument("--dataset", type=str, default="MedQA", help="Specify dataset name")
    # 选择多选题还是自由问答题，mc or ff,
    ...
    method = "Colacare_250918" # ColaCare by default
