    ...
    <div class="flex flex-wrap items-end justify-between gap-3">
      <div class="w-full max-w-md">
          ...
          <div class="w-56">
            <AppSelect
              ...
              :options="[
                ...
                { value: 'Annotator_3', label: 'Annotator #3' },
              ]"
            />
          </div>
          <div class="flex-1">
        ...
        <div class="mt-1 text-xs text-slate-600">
          Auto-saved to <span class="font-mono">localStorage</span> on every change.
        </div>
      </div>

      <div class="flex flex-wrap items-center gap-3">
        <div class="text-xs text-slate-600">
          Done: <span class="font-semibold text-slate-900">{{ doneCount }}</span> / {{ OPEN_CODING_CASES.length }}
        </div>
        <AppButton variant="secondary" :disabled="!annotatorName.trim()" @click="exportJson">Export JSON</AppButton>
      </div>
    </div>

    <div class="grid gap-4 lg:grid-cols-[360px_1fr_420px]">
      <AppCard class="p-4">
        <div class="max-h-[75vh] overflow-auto pr-1">
          <div class="space-y-2">
            <button
      ...
      <AppCard v-if="activeCase" class="sticky top-[76px] h-fit p-5">
        <div class="space-y-4">
      ...
      <AppCard v-else class="p-5 lg:col-span-2">
