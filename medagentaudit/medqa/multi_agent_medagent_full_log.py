...
class BaseAgent:
    ...
    def __init__(self,
        ...
        # ***** MODIFICATION START *****
        # 为每个Agent实例添加一个日志记录器，用于存储其完整的输入和输出历史
    ...
    def call_llm(self,
        ...
        while retries < max_retries:
            try:
                ...
                self.interaction_log.append(request_log)
                # ***** MODIFICATION END *****
                return response
...
class ExpertGathererAgent(BaseAgent):
    ...
    def gather_question_domain_experts(self, question: str) -> List[MedicalSpecialty]:
        """
        Gather relevant domain experts for a medical question.  at least 3 experts.
...
class MDTConsultation:
    ...
    def run_consultation(self,
        ...
        # ***** MODIFICATION START *****
        # Case consultation history - 结构优化，增加更多用于分析的字段
        case_history = {
            "qid": qid,
            "question": question,
            "options": options,
            "image_path": image_path,
            "expert_gatherer_log": self.expert_gatherer.interaction_log, # 记录ExpertGatherer的交互
            "selected_specialties": [s.value for s in specialties],
            "rounds": [],
            "final_decision": None,
            "termination_reason": None, # 新增字段，记录循环终止原因
        ...
        while current_round < self.max_rounds and not consensus_reached:
            ...
            round_data = {
                ...
                "doctor_analyses": [],
                "meta_agent_synthesis": {
                    "synthesis_content": None,
                    "interaction_log": None
                },
                "doctor_reviews": [],
                "consensus_status_after_round": False
            }
            # ***** MODIFICATION END *****
            ...
            for i, doctor in enumerate(self.doctor_agents):
                ...
                # ***** MODIFICATION START *****
                # 记录每个医生详细的分析过程
                round_data["doctor_analyses"].append({
                    "doctor_id": doctor.agent_id,
                    "specialty": doctor.specialty.value,
                    "analysis_result": opinion,
                    "interaction_log": doctor.interaction_log[-1] # 记录本次分析的交互日志
                })
                # ***** MODIFICATION END *****
            ...
            # ***** MODIFICATION START *****
            # 记录MetaAgent的合成过程
            round_data["meta_agent_synthesis"]["synthesis_content"] = synthesis
            round_data["meta_agent_synthesis"]["interaction_log"] = self.meta_agent.interaction_log[-1]
            # ***** MODIFICATION END *****
            ...
            for i, doctor in enumerate(self.doctor_agents):
                ...
                # ***** MODIFICATION START *****
                # 记录每个医生详细的审查过程
                round_data["doctor_reviews"].append({
                    "doctor_id": doctor.agent_id,
                    "specialty": doctor.specialty.value,
                    "review_result": review,
                    "interaction_log": doctor.interaction_log[-1] # 记录本次审查的交互日志
                })
                # ***** MODIFICATION END *****
            ...
            # ***** MODIFICATION START *****
            # 记录本轮结束后的共识状态
            round_data["consensus_status_after_round"] = all_agree
            # ***** MODIFICATION END *****
            ...
            if all_agree or current_round == self.max_rounds:
                ...
                if all_agree:
                    ...
                    # ***** MODIFICATION START *****
                    case_history["termination_reason"] = "ConsensusReached"
                    # ***** MODIFICATION END *****
                else:
                    print("Max rounds reached without consensus")
                    # ***** MODIFICATION START *****
                    case_history["termination_reason"] = "MaxRoundsReached"
                    # ***** MODIFICATION END *****
        ...
        print(f"Final answer: {final_decision.get('answer', '') if final_decision else 'N/A'}")

        # Calculate processing time
        processing_time = time.time() - start_time

        # ***** MODIFICATION START *****
        # Add final decision to history with its own detailed log
        if final_decision:
            case_history["final_decision"] = {
                "decision_content": final_decision,
                "interaction_log": self.decision_agent.interaction_log[-1]
            }
        # ***** MODIFICATION END *****
...
def main():
    ...
    parser.add_argument("--model", required=True, type=str, default="gemini-2.5-flash",
                       help="Model used for doctor agents")
    parser.add_argument("--meta_model", required=True, type=str, default="gemini-2.5-flash",
                       help="Model used for meta agent")
    parser.add_argument("--decision_model", required=True, type=str, default="gemini-2.5-flash",
    ...
    for item in tqdm(data[:args.num], desc=f"Running MDT consultation on {dataset_name}"):
        ...
        try:
            ...
            # ***** MODIFICATION START *****
            # 优化最终保存的JSON结构，使其更清晰
            predicted_answer_content = result.get("final_decision", {}).get("decision_content", {}).get("answer", "Error: No final answer generated")
            ...
            item_result = {
                ...
                "model_settings": {
                    "doctor_agent_model": args.model,
                    "meta_agent_model": args.meta_model,
                    "decision_agent_model": args.decision_model,
                },
                "predicted_answer": predicted_answer_content,
                "full_consultation_log": result # 将完整的、结构化的case_history作为日志主体
            }
            # ***** MODIFICATION END *****
            ...
            # ***** MODIFICATION START *****
            # 即使处理失败，也记录错误日志，便于分析
            error_log_path = os.path.join(logs_dir, f"{qid}-error.json")
            error_data = {
                "qid": qid,
                "timestamp": int(time.time()),
                "error_message": str(e),
            ...
            save_json(error_data, error_log_path)
            # ***** MODIFICATION END *****
